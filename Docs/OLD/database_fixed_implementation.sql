-- Fixed Database Implementation Script for RetailVerse System
-- This script addresses the PostgreSQL errors from the previous run
...
-- Fix pack_size column type if it's VARCHAR
DO $$ 
BEGIN
  IF EXISTS (
    ...
    ALTER TABLE products ALTER COLUMN pack_size TYPE INTEGER USING pack_size::INTEGER;
  END IF;
END $$;
...
-- Phase 5: Drop and Recreate Enhanced Fit Score Function
DROP FUNCTION IF EXISTS calculate_enhanced_fit_score(INTEGER, INTEGER);

CREATE FUNCTION calculate_enhanced_fit_score(
  p_brand_id INTEGER,
...
BEGIN
  ...
  total_score := ROUND(
    (category_match_score * 0.30) +
    (subcategory_match_score * 0.30) +
    (margin_match_score * 0.10) +
    (asp_match_score * 0.30)
  ) * 100;
...
-- Phase 7: Drop and Recreate Views for Easy Data Access
DROP VIEW IF EXISTS brand_with_categories;
CREATE VIEW brand_with_categories AS
SELECT 
  b.*,
  json_agg(
    json_build_object(
...
DROP VIEW IF EXISTS products_with_specs;
CREATE VIEW products_with_specs AS
SELECT 
  p.*,
  b.brand_name,
  b.official_email
