...
console.log('Looking for file at:', excelFilePath);

try {
    // Check if file exists
    const fs = require('fs');
    if (!fs.existsSync(excelFilePath)) {
        console.log('❌ File not found at:', excelFilePath);
        console.log('Available files in Docs/New/:');
        const newDir = path.join(__dirname, '..', 'Docs', 'New');
        if (fs.existsSync(newDir)) {
            const files = fs.readdirSync(newDir);
            files.forEach(file => console.log('  -', file));
        } else {
            console.log('Docs/New directory does not exist');
        }
        return;
    }
